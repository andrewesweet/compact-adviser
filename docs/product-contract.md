...
- Installing or loading the package is consent to send eligible checkpoint context to TypeSafe when a key is available and other product gates pass (mode, minimum context, idle session, and so on). Pi and Claude Code let a TypeSafe API key be entered in the compact-adviser menu. Codex also provides an external settings CLI. Grok's model-mediated command never accepts a key; one may be saved only by running its CLI from an external shell. After save the UI reports only presence or key source (`env` / `saved` / `.env` / `missing`), never the value. A non-empty launch-environment value wins over the saved key, which wins over a cwd `.env`.
